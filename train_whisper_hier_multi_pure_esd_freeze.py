...
def run(rank, n_gpus, hps):
  ...
  for name, param in net_g.named_parameters():
      ...
      if "dec" in name:
          #print("freezing",name)
          param.requires_grad = False
      # if "enc_q" in name:#我突然意识到，enc_q最后并不会被用于推理，是可以随便finetune而不会影响说话人相似性的啊。
      #     #print("freezing",name)
      #     param.requires_grad = False
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  net_g.train()
  net_d.train()
  for batch_idx, (x, x_lengths, spec, spec_lengths,weo,weo_lengths, y, y_lengths,ref_mel, lang_id) in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        #loss_gen_all = 20+loss_dur + loss_kl_text_to_whisper+loss_kl_text_to_spec+loss_kl_whisper_to_spec
    optim_g.zero_grad()
