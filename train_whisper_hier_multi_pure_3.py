...
from text_cn.symbols import symbols#跨语种的symbol
...
def run(rank, n_gpus, hps):
  ...
  train_sampler = DistributedBucketSampler(
      ...
      [50,100,150,200,250,300,350,400,450,500,600],#,700,800,900,1000#600除以16000乘以320等于12，而1000除以22050诚意256等于11.6，这波赢了啊
      num_replicas=n_gpus,
      rank=rank,
      shuffle=True)
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths,weo,weo_lengths, y, y_lengths,ref_mel, lang_id) in enumerate(train_loader):#这里是需要改的，包括langid和refmel
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_kl_text_to_whisper = kl_loss(z_p_weo, logs_q_weo,m_p, logs_p,  z_mask_weo) * hps.train.c_kl * hps.train.c_kl_weo2text#0.1
        loss_kl_whisper_to_spec = kl_loss(z_p, logs_q,m_q_weo, logs_q_weo,  z_mask) * hps.train.c_kl * hps.train.c_kl_spec2weo#1
        loss_kl_text_to_spec= kl_loss(z_p_p, logs_q, m_p, logs_p, z_mask) * hps.train.c_kl * hps.train.c_kl_spec2text#0.1
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        print("0.1,1,0.1")
        losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_dur, loss_kl_text_to_whisper/0.1,loss_kl_whisper_to_spec/1,loss_kl_text_to_spec/0.1]#
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    y_hat_vc,  mask_vc, *_ = generator.voice_conversion(weo, weo_lengths, mel=ref_mel_vc,lang=lang_id,max_len=1000)#.module.infer(x, x_lengths, mel=ref_mel,lang=lang_id,max_len=1000)
    y_hat_lengths_vc = mask_vc.sum([1,2]).long() * hps.data.hop_length

    
    audio_dict.update({"gen/audio_vc": y_hat_vc[0,:,:y_hat_lengths_vc[0]]})
    #if global_step == 0:
    # image_dict.update({"gt/mel": utils.plot_spectrogram_to_numpy(mel[0].cpu().numpy())})
    audio_dict.update({"gt/audio_vc(ref)": y_vc[0,:,:y_lengths_vc[0]]})
