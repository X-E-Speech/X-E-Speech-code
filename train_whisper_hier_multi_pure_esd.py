  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths,weo,weo_lengths, y, y_lengths,ref_mel, lang_id) in enumerate(train_loader):#这里是需要改的，包括langid和refmel
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_kl_text_to_whisper = kl_loss(z_p_weo, logs_q_weo,m_p, logs_p,  z_mask_weo) * hps.train.c_kl * hps.train.c_kl_weo2text#0.4
        loss_kl_whisper_to_spec = kl_loss(z_p, logs_q,m_q_weo, logs_q_weo,  z_mask) * hps.train.c_kl * hps.train.c_kl_spec2weo#0.45
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_dur, loss_kl_text_to_whisper/0.4,loss_kl_whisper_to_spec/0.25,loss_kl_text_to_spec/1]#
